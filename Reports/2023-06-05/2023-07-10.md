...
```
- Me and Professor Clark troubleshot changing the screen to windowed and gettings its resolution to stay at a smaller scale of 700x500 which we had to do in the DefaultGameUserSettings.ini
- After me and Anjali finished addressing all the new suggestions we had a small code review with Professor Clark where we were able to start working on getting boxnav ready to work with ue5osc. We had a few issues getting set up and getting ue5osc downloadable to be used, but we eventually got to the point where we realized we would need to change ue5osc method names to match the older ones
...
- Try to add own configuration to set the ip and ports so we don't have to repackage
