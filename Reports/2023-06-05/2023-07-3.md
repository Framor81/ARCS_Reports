...
- Decided trying to turn an image into a 2D texture and then into a binary array was too time consuming and instead shifted my focus in finding a workaround.
    ...
    - My thought process was by making sure we always send the images to the same place with a common naming convention which will allow us to request the image from the dir we just saved them in.  
-  I was able to implement this idea with an if statement that checked your system os, and assigned two different possible paths. One for windows and one for Mac. Clark then recommended I change it into an arguemtn the user has to give through the command line.
- Then we added an arguemnt parser to require the user to have to give a path where they want the image to be downloaded. This allowed us to ensure no matter what device you are on the python OSCSender is able to function as intended. 
...
- I finalized my blueprint and prepared how I wanted to explain it https://blueprintue.com/blueprint/9rm6eoja/.
- Me and Anjali then heavily discussed our ideas for how we should continue working in unreal engine, modeling, and our goals for our future as coworkers. We decided we would work more with peer programming and collaborative work. We would try to finalize our work in unreal engine and python to ensure we can start the collecting data portion of our task.
- I then committed the OSC files into github.

## Friday

- I also read the research journal Christy had linked
- I wrote a detailed section about the unreal Engine Blueprint 
